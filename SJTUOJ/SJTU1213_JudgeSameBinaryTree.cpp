...
private:
    ...
    bool isSame(Node *b,Node *a) const;
public:
...
template <class T>
bool BinaryTree<T>::isSame(Node *b,Node *a) const
{
    if (b == NULL && a == NULL) return true;
    if (b == NULL && a != NULL) return false;
    if (b != NULL && a == NULL) return false;
    if (b->data != a->data) return false;
    else return isSame(b->left,a->left) && isSame(b->right,a->right);
}

template <class T>
bool BinaryTree<T>::isSame(BinaryTree<T> b)
{
    return isSame(root,b.root);
}

int main()
{
    BinaryTree<int> test1,test2;
    int n1,n2;
    cin >> n1;
    test1.CreateTree(n1);
    cin >> n2;
    test2.CreateTree(n2);
    if (test1.isSame(test2)) cout << 'Y';
    else cout << 'N';
}
